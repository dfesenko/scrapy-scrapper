...
class MorganlewisSpider(scrapy.Spider):
    ...
    @staticmethod
    def generate_paging_url(page_num, num_per_page=100):
    ...
    def get_services(response):
        ...
        first_block_person_depart_info_head = response.css('section.person-depart-info:first-of-type h2 ::text').get()
    ...
    def parse(self, response):
        ...
        # follow next page
        last_page_num = int(response.css('div.c-pagination a.last ::attr(data-pagenum)').get())
        current_page_num = int(response.css('div.c-pagination a.selected ::attr(data-pagenum)').get())
        if current_page_num < last_page_num:
            response.follow(self.generate_paging_url(page_num=current_page_num + 1), callback=self.parse)

    def parse_profile(self, response):
        profile_data = {
            'url': response.request.url,
            'photo_url': self.get_absolute_photo_url(response),
        ...
        yield scrapy.Request(url="https://www.morganlewis.com/api/sitecore/accordion/getaccordionlist",
                             ...
                             body=json.dumps({'itemId': '{' + self.get_person_id(response) + '}',
